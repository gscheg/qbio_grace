...
#download survminer package
#survival analysis
library(arsenal) #What do you need to do before running this line? 
#download arsenal package
clin_query <- GDCquery(project = "TCGA-BRCA", data.category="Clinical", file.type="xml", barcode = barcodes_clinic)
...
#allows you to summarize data as counts/percentages
library(tableone)
clinicalsummary <- CreateTableOne(data = clinic)
subtypes <- TCGAquery_subtype(tumor = "BRCA") #creating a new separate dataframe with all patients with BRCA tumors
# Add the age_category column in the same way as above
age_subtypes = subtypes$age_at_initial_pathologic_diagnosis
subtypes$age_category <- ifelse(age_subtypes < 40, "Young", ifelse(age_subtypes >= 60, "Old", "Mid"))
...
names(subtypes) = make.names(names(subtypes)) #cleans up names in the subtypes
table_arse <- tableby(age_category ~ (pathologic_stage)+ (mRNA.Clusters)+
         (BRCA_Pathology), data=subtypes,
         numeric.test="kwt", cat.test="chisq", numeric.stats = c("Nmiss", "meansd"), total=FALSE)
...
str(clinic)
#TCGAanalyze_survival() creates a survival plot from clinical data
#uses Kaplan Meier
...
#creates a new col describing the overall survival for patients
...
ggcoxadjustedcurves(fit, data=clinic)
dev.off()
